...
export default function BuilderPage() {
    ...
    const fetchContentTypes = async () => {
        try {
            const res = await fetch(`/api/content-types?orgSlug=${orgSlug}&projectSlug=${projectSlug}`)
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/content-types", {
                ...
                body: JSON.stringify({ ...newType, orgSlug, projectSlug }),
            })
                                        ...
                                        <TableCell className="text-right">
                                            <div className="flex justify-end gap-2">
                                                <Button
                                                    variant="ghost"
                                                    size="icon"
                                                    onClick={() => router.push(`/dashboard/${orgSlug}/projects/${projectSlug}/builder/${type.id}`)}
                                                >
